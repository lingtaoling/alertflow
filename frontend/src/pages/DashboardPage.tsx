...
import {
  Plus, RefreshCw, LogOut, Zap, AlertCircle, CheckCircle2, Activity,
  Loader2, ChevronLeft, ChevronRight, Building2, User, Shield
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const { org, user, orgId, userId } = useAppSelector((s) => s.auth);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="border-b border-ink-200 bg-white/95 backdrop-blur-md sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          {/* Tenant context — shows how isolation is enforced */}
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-6">
        ...
        <div className="flex items-center gap-1 mb-4 flex-wrap">
          ...
          <span className="ml-auto text-ink-500 text-xs">{total} alert{total !== 1 ? 's' : ''}</span>
        </div>
