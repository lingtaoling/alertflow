...
export default function AlertDetailModal({ alert }: Props) {
  ...
  async function handleAdvance() {
    ...
    if (!next) return;
    await dispatch(updateAlertStatus({ id: alert.id, status: next }));
    dispatch(fetchAlertEvents(alert.id));
  }
  ...
  const eventStatusColors: Record<AlertStatus, string> = {
    NEW: 'border-signal-orange bg-signal-orange/20 text-signal-orange',
    ACKNOWLEDGED: 'border-signal-amber bg-signal-amber/20 text-signal-amber',
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            {event.fromStatus ? (
                              <div className="flex items-center gap-1.5 text-xs">
                                <span className={`badge ${statusConfig[event.fromStatus].badge} py-0.5`}>{statusConfig[event.fromStatus].label}</span>
                                <ArrowRight size={10} className="text-ink-500" />
                                <span className={`badge ${statusConfig[event.toStatus].badge} py-0.5`}>{statusConfig[event.toStatus].label}</span>
                              </div>
                            ) : (
            ...
            {next && (
              <button
                className={`btn-primary w-full justify-center ${
                  next === 'RESOLVED' ? '!bg-signal-green hover:!bg-emerald-600' : ''
                }`}
